/*
g++ -std=c++17 -o ceres_08_ba_eigen_projection ./ceres_08_ba_eigen_projection.cpp \
...
struct ReprjError {
    ...
    bool operator()(const T* intrinsics,
                    ...
                    T* residual
                        ...
                        Matrix<T, 3, 1> t(cam_trans[0], cam_trans[1], cam_trans[2]);
                        Matrix<T, 4, 1> P_homo(point3D[0], point3D[1], point3D[2], T(1));
                        Matrix<T, 3, 4> prj_mat;
                        prj_mat.template block<3, 3>(0, 0) = R;
                        // prjMat.block(0, 0, 3, 3) = R;
                        prj_mat.template block<3, 1>(0, 3) = t;
                        // prjMat.block(0, 3, 3, 1) = t;
                        prj_mat = K * prj_mat;
                        Matrix<T, 3,1> prj_p;
                        prj_p = prj_mat * P_homo;
                        prj_p(0, 0) = prj_p(0, 0) / prj_p(2, 0);
                        prj_p(1, 0) = prj_p(1, 0) / prj_p(2, 0);
                        residual[0] = T(measured_u) - prj_p(0, 0);
                        residual[1] = T(measured_v) - prj_p(1, 0);
